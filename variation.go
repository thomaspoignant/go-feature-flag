...
// An error is return if you don't have init the library before calling the function.
// If the key does not exist we return the default value.
func JSONArrayVariation(flagKey string, ctx ffcontext.Context, defaultValue []any) ([]any, error) {
...
func (g *GoFeatureFlag) JSONArrayVariationDetails(
	...
	res, err := getVariation[[]any](g, flagKey, ctx, defaultValue, "[]interface{}")
	notifyVariation(g, flagKey, ctx, res)
	return res, err
}
...
// An error is return if you don't have init the library before calling the function.
// If the key does not exist we return the default value.
func JSONVariationDetails(flagKey string, ctx ffcontext.Context, defaultValue map[string]any,
...
func (g *GoFeatureFlag) JSONVariationDetails(
	...
	res, err := getVariation[map[string]any](g, flagKey, ctx, defaultValue, "bool")
	notifyVariation(g, flagKey, ctx, res)
	return res, err
}
...
// Note: Use this function only if you are using multiple go-feature-flag instances.
func (g *GoFeatureFlag) RawVariation(flagKey string, ctx ffcontext.Context, sdkDefaultValue any,
) (model.RawVarResult, error) {
	res, err := getVariation[any](g, flagKey, ctx, sdkDefaultValue, "interface{}")
	notifyVariation(g, flagKey, ctx, res)
...
func constructMetadata(f flag.Flag, resolutionDetails flag.ResolutionDetails) map[string]any {
