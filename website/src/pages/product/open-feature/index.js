...
import openfeatureArch from '@site/static/docs/openfeature/openfeature-arch.png';
import concepts from '@site/static/docs/openfeature/concepts.png';

const PAGE_TITLE = 'OpenFeature support';
const PAGE_DESCRIPTION =
  'GO Feature Flag is built on OpenFeature, the CNCF open standard for feature flagging. We ship OpenFeature providers instead of custom SDKs, support OFREP, and help steer the standard - so you stay vendor-neutral.';

const OPENFEATURE_URL = 'https://openfeature.dev';
const SDK_DOCS = '/docs/sdk';
const OFREP_DOCS = '/docs/sdk/ofrep';
const RELAY_PROXY_DOCS = '/docs/relay-proxy/';
const GOVERNANCE_BLOG = '/blog/2026/04/09/openfeature-governance-committee';
...
// OpenFeature wordmark, reused from the previous version of this page.
// Uses currentColor so it adapts to light and dark mode.
function OpenFeatureLogo({className}) {
  return (
    <svg
...
const STANDARD_CARDS = [
  ...
  {
    icon: <FaUsers />,
    title: 'Backed by the CNCF community',
    description:
...
const COVERAGE_CARDS = [
  {
    ...
    link: SDK_DOCS,
    linkLabel: 'Server SDKs',
  },
  {
    icon: <FaDesktop />,
    title: 'Client-side providers',
    description:
      'JavaScript, TypeScript, React, Angular, Swift, and Android (Kotlin) for evaluating flags in the browser and on mobile.',
    link: SDK_DOCS,
    linkLabel: 'Client SDKs',
  },
  {
    icon: <FaNetworkWired />,
    title: 'OFREP for everything else',
    description:
      'The OpenFeature Remote Evaluation Protocol is a standard REST contract any language can call - so even stacks without a native provider can still talk to GO Feature Flag.',
    link: OFREP_DOCS,
    linkLabel: 'About OFREP',
  },
];
...
const FAQ_ITEMS = [
  ...
  {
    question: 'Why does GO Feature Flag not have its own SDK?',
    answer:
  ...
  {
    question: 'Can I switch feature flag providers later?',
    answer:
  ...
  {
    question: 'Is GO Feature Flag involved in OpenFeature itself?',
    answer:
...
export default function OpenFeatureSupportPage() {
  ...
  const pageUrl = `${siteUrl}/product/open-feature`;
  ...
  return (
    <Layout
      ...
      keywords={[
        ...
        'vendor neutral feature flags',
        'OFREP',
        'CNCF feature flagging',
        'OpenFeature SDK',
        'GO Feature Flag OpenFeature',
      ]}>
      <Head>
      ...
      <Title
        ...
        description="GO Feature Flag is built on OpenFeature - the open standard for feature flagging. You write against a vendor-neutral API, not our proprietary client, so you are never locked in."
        actions={[
          {
            label: 'Get started',
            href: SDK_DOCS,
          ...
          {
            label: 'OpenFeature.dev',
            href: OPENFEATURE_URL,
      ...
      <FeatureRow
        ...
        imageSrc={openfeatureArch}
        imageAlt="OpenFeature sits between your application and a feature flag backend: your app calls the standard OpenFeature SDK, and a provider connects that SDK to a specific service - here a generic 'Flags-R-us' vendor.">
        <p className={prose}>
          <Link to={OPENFEATURE_URL}>OpenFeature</Link> is an open standard for
          feature flag management, hosted by the{' '}
          <strong>Cloud Native Computing Foundation (CNCF)</strong>. It defines
          a single, vendor-neutral API and a set of SDKs so developers can use
          feature flags in a consistent way, whatever backend powers them.
        </p>
        <p className="mb-0">
          The key idea is the <strong>provider</strong>: your application talks
          only to the OpenFeature SDK, and a provider plugs a specific backend
          into that standard API. Change the provider and your evaluation code
          stays exactly the same.
        </p>
      </FeatureRow>

      <Cards
        title="Why building on an open standard matters"
      ...
      <FeatureRow
        ...
        title="We ship providers, not a custom SDK"
        media={
          <div className="mx-auto flex max-w-xs items-center justify-center px-6 text-gray-800 dark:text-gray-100">
            <OpenFeatureLogo className="h-auto w-full" />
          </div>
        }>
        <p className="mb-4">
          At GO Feature Flag we believe in open standards and vendor neutrality
        ...
        <p className="mb-0">
          GO Feature Flag has been committed to the standard since
          OpenFeature&rsquo;s early days. We ship native providers, support{' '}
          <Link to={OFREP_DOCS}>OFREP</Link> (the OpenFeature Remote Evaluation
          Protocol), and follow the spec closely so we stay compatible across
          the whole ecosystem. The payoff for you: adopt feature flags fast,
          with no lock-in and no proprietary infrastructure to learn.
        </p>
      </FeatureRow>
      ...
      <FeatureRow
        ...
        title="OpenFeature SDK, GO Feature Flag provider, relay proxy"
        reverse
        imageSrc={concepts}
        imageAlt="Your application uses the OpenFeature SDK with a GO Feature Flag provider; the provider talks to the GO Feature Flag relay proxy hosted in your infrastructure, which reads your flag configuration from the store of your choice.">
        <p className="mb-4">
          In your code you use the standard <strong>OpenFeature SDK</strong>{' '}
          together with a <strong>GO Feature Flag provider</strong>. The
          provider connects to the{' '}
          <Link to={RELAY_PROXY_DOCS}>relay proxy</Link> you host in your own
          infrastructure, which reads your flag configuration from the store of
          your choice.
        </p>
        <p className="mb-0">
          Evaluation runs in one of two modes:{' '}
          <strong>remote evaluation</strong> (the provider asks the relay proxy
          for each evaluation) or <strong>in-process evaluation</strong> (the
          provider syncs the configuration and evaluates locally, with no
          network call on the hot path). Either way, the same evaluation data is
          collected for observability.
        </p>
      </FeatureRow>
      <section className="px-6 pb-12 max-w-[1400px] mx-auto">
        <p className="mx-auto max-w-3xl text-center">
          <Link className={docLink} to={SDK_DOCS}>
      ...
      <Cards title="Use it in your language" cards={COVERAGE_CARDS} />

      {/* Leadership / steering the standard */}
      <section className="px-6 py-12 max-w-[1400px] mx-auto text-center">
        ...
        <p className="mx-auto max-w-3xl text-lg leading-relaxed text-gray-700 dark:text-gray-300">
          Our commitment goes beyond implementing the spec - we help shape it.
          GO Feature Flag&rsquo;s creator, <strong>Thomas Poignant</strong>,
          served on the OpenFeature <strong>Technical Committee</strong> and was
          elected to the OpenFeature <strong>Governance Committee</strong> for
          the 2026-2028 term. That means the perspective of a maintainer who
          runs feature flagging in production feeds directly into where the
          standard goes - and that GO Feature Flag and OpenFeature stay in sync.
        </p>
        <p className="mt-6">
          <Link className={docLink} to={GOVERNANCE_BLOG}>
      ...
      <CtaBand
        title="Start with OpenFeature and GO Feature Flag"
        description="Self-hosted, OpenFeature-native, MIT-licensed. Use the standard SDKs, drop in a GO Feature Flag provider, and stay vendor-neutral from day one."
        actions={[
          {
            label: 'Get started',
            href: SDK_DOCS,
