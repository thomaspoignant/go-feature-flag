  ...
  const CustomForm = ({status, message, onValidated}) => {
    ...
    return (
      <div>
        ...
        />
        <button
          className="pushy__btn pushy__btn--df pushy__btn--black"
          onClick={submit}>
          Subscribe <i className="fa-regular fa-paper-plane"></i>
        </button>
        {status === 'error' && (
          <div
            className={clsx(styles.newsletterMessage, styles.error)}
            dangerouslySetInnerHTML={{__html: message}}
          />
        )}
        {status === 'success' && (
          <div
            className={clsx(styles.newsletterMessage, styles.success)}
            dangerouslySetInnerHTML={{__html: message}}
          />
        )}
      </div>
