...
In all the details pages of the notifiers below you will have an example for both examples.

## Available Notifiers
Below is the full list of notifiers that are available in GO Feature Flag.

<div className={"grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4"}>
  {getIntegrations("v1.55.2").notifiers.filter(item => item.docLink !== undefined).map((item) => {
    return (
      <Card
        logo={item.logo}
...
## Custom notifier
If you have a specific use case that is not covered by the built-in notifiers, you can create your own custom notifier.
...
:::
:::warning
If you are using a custom notifier, it is easy to use it for the GO Module, but if you want to use it in the relay proxy you will have to recompile it yourself.
:::