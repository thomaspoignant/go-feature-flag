...
| `clientCertPath` | <NotMandatory /> | String | **none** | Path to the client certificate file used for mTLS. Must be configured with `clientKeyPath`.            |
| `clientKeyPath`  | <NotMandatory /> | String | **none** | Path to the client certificate key file used for mTLS. Must be configured with `clientCertPath`.       |
| `caCertPath`     | <NotMandatory /> | String | **none** | Path to the CA certificate file used to verify the server certificate.                                 |
...
| __ClientCertPath__ | <NotMandatory /> | Path to the client certificate file used for mTLS. Must be configured with `ClientKeyPath`.              |
| __ClientKeyPath__  | <NotMandatory /> | Path to the client certificate key file used for mTLS. Must be configured with `ClientCertPath`.         |
| __CACertPath__     | <NotMandatory /> | Path to the CA certificate file used to verify the server certificate.                                   |
