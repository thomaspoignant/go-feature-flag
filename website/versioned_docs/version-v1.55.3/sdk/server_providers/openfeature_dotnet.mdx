...
If your application is orchestrated by [.NET Aspire](https://aspire.dev), you don't have to
configure the provider by hand. The `CommunityToolkit.Aspire.GoFeatureFlag` client integration
registers a `GOFeatureFlagProvider` in the dependency injection container and adds a health check
for you, and the relay proxy endpoint comes straight from your AppHost, so there is no hardcoded
...
The `connectionName` must match the name of the GO Feature Flag resource declared in your AppHost.
...
If you need several relay proxies, use `AddKeyedGoFeatureFlagClient` to register one provider per
connection name and resolve them with `[FromKeyedServices("...")]`.

:::note
To learn how to model the relay proxy in your AppHost, see the
[.NET Aspire deployment page](../../relay-proxy/deployment/aspire) and the upstream
