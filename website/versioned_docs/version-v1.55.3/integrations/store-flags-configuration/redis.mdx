...
We expect the flag to be stored as a `string:string` format where the key is the flag key (with or without a prefix) and the value is a string representing the flag in JSON.
