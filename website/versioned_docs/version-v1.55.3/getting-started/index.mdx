...
Start with [**What are feature flags?**](/product/what-are-feature-flags) for a quick primer on how they work, then come back here to build your first one.
:::
...
:::info
If you cannot reach Docker Hub, the same image is published to the GitHub Container
Registry as `ghcr.io/go-feature-flag/go-feature-flag` for the releases after `v1.55.1`.
:::
...
An SDK-compliant implementation that resolves flag values from a particular flag management system, allowing the use of the Evaluation API as an abstraction for the system in question.
...
```
**What is happening in this new code?**
- We are creating an evaluation context with a unique identifier of the user coming through the `X-USER-ID` header.  
...
- We are calling the OpenFeature API to evaluate the flag `show-email-contact`, with the following parameters:
  ...
  - `defaultValue`: the default value if we have any error, the goal is to have a safe fallback value, whatever happens.
  - `evaluationContext`: the context of the evaluation, in this case, the unique identifier of the user.
- In return of the evaluation, we are guaranteed to have a value for the flag, so we can use it to decide if we display the email of the user in the response.

**🎉 You have successfully added your first feature flag in your application.**
...
Now that you've been able to create your first feature flag, configure the relay proxy, and use it in your GO API, here are some pieces of documentation you may want to check:
...
- [**Store your flags’ configuration**](./integrations/store-flags-configuration): Learn where to store your feature flags configuration.
