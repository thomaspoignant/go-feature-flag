...
**GO Feature Flag** supports a variety of retrievers out of the box _(see list below)_.

You have 2 different ways to specify a retriever depending on if you are using the **GO Module** or the **Relay Proxy**.
In all the detail pages of the retrievers below you will have an example for both examples.
...
<div className={"grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4"}>
  {getIntegrations("v1.54.1").retrievers.filter(item => item.docLink !== undefined).map((retriever) => {
    return (
      <Card
        logo={retriever.logo}
...
GO Feature Flag will try to call the Retrieve function only if the status is `RetrieverReady`.
