...
<div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-8">
  ...
  title="One set of feature flags"
   description="This is the default way of using GO Feature Flag, it requires a minimum configuration and is easy to setup"
    />

<Vignette 
icon="fa-users" 
color="blue" 
link="#configure-multiple-flag-sets"
title="Use multiple flag sets"
description="Separate your feature flags into different flag sets to give ownership to different teams or different usage."
/ >
</div>
...
- A human-readable name for the flag set. If not provided, a UUID will be generated.

#### `apiKeys` (mandatory)
- **type**: []string
- **mandatory**: <Mandatory />
- List of API keys that can access this flag set. Each API key must be unique across all flag sets.

#### `retriever` / `retrievers`
- **type**: [retriever](#type-retriever) / [][]retriever
- **mandatory**: <Mandatory />
- Configuration for retrieving flag configurations. Supports the same options as the main configuration.

#### `exporter` / `exporters`
- **type**: [exporter](#type-exporter) / [][]exporter
- **mandatory**: <NotMandatory />
- Configuration for exporting evaluation data. Supports the same options as the main configuration.

#### `notifier` / `notifiers`
- **type**: [notifier](#type-notifier) / [][]notifier
- **mandatory**: <NotMandatory />
...
- **default**: `yaml`
- **mandatory**: <NotMandatory />
- Format of the flag configuration files (yaml, json, toml).

#### `pollingInterval`
- **type**: int
- **default**: `60000`
- **mandatory**: <NotMandatory />
- Polling interval in milliseconds for retrieving flag updates.

#### `startWithRetrieverError`
- **type**: boolean
- **default**: `false`
- **mandatory**: <NotMandatory />
- Whether to start the flag set even if the retriever fails initially.

#### `enablePollingJitter`
- **type**: boolean
- **default**: `false`
- **mandatory**: <NotMandatory />
- Whether to add jitter to polling intervals to avoid thundering herd problems.

#### `disableNotifierOnInit`
- **type**: boolean
- **default**: `false`
- **mandatory**: <NotMandatory />
- Whether to disable notifications when flags are loaded for the first time.
...
When using flag sets, all evaluation requests must include an API key in the `Authorization` header:

```bash
curl -H "Authorization: Bearer team-a-api-key-1" \
     -H "Content-Type: application/json" \
     -d '{"targetingKey": "user123", "flagKey": "my-feature"}' \
     http://localhost:1031/v1/feature/my-feature/eval
```
...
If you're currently using the single configuration mode and want to migrate to flag sets:

1. **Create flag sets**: Move your existing configuration into one or more flag sets
2. **Add API keys**: Each flag set needs at least one API key
3. **Update clients**: Add the appropriate API key to your client requests
4. **Test**: Verify that evaluations work correctly with the new API key-based routing
...
flagSets:
  ...
  - name: staging
    ...
    retrievers:
      - kind: file
        path: /flags/staging-flags.yaml
    exporters:
      - kind: s3
        bucket: staging-evaluations
  ...
  - name: production
    ...
    retrievers:
      - kind: s3
        bucket: prod-feature-flags
        item: flags.yaml
    exporters:
      - kind: kafka
        topic: prod-evaluations
    notifiers:
      - kind: slack
...
#### `flagSet.retriever` / `flagSet.retrievers`
Configuration for retrieving flag configurations. Supports the same options as the main configuration.

- option name: `retriever` / `retrievers`
- type: **[retriever](#type-retriever)** / **[[]retriever](#type-retriever)**
- default: **none**
- mandatory: <Mandatory />

#### `flagSet.exporter` / `flagSet.exporters`
Configuration for exporting evaluation data. Supports the same options as the main configuration.

- option name: `exporter` / `exporters`
- type: **[exporter](#type-exporter)** / **[[]exporter](#type-exporter)**
- default: **none**
- mandatory: <NotMandatory />

#### `flagSet.notifier` / `flagSet.notifiers`
Configuration for flag change notifications. Supports the same options as the main configuration.

- option name: `notifier` / `notifiers`
- type: **[notifier](#type-notifier)** / **[[]notifier](#type-notifier)**
- default: **none**
- mandatory: <NotMandatory />
