...
<ul>
{ getIntegrations("").retrievers.filter(r => r.docLink !== undefined).map((r) => (
...
**Available exporters**:
<ul>
  { getIntegrations("").exporters.filter(r => r.docLink !== undefined).map((r) => (
...
**Available notifiers**:
<ul>
  { getIntegrations("").notifiers.filter(r => r.docLink !== undefined).map((r) => (
