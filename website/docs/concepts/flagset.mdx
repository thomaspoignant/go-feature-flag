...
This is useful when you need to rotate API keys or add new keys to an existing flag set while the service is running.
...
flagSets:
  - name: team-a  # ✅ This flagset supports runtime API key updates
    apiKeys:
      - "team-a-api-key"
    retrievers:
      - kind: file
...
```yaml title="goff-proxy.yaml"
flagSets:
  - apiKeys:  # ⚠️ No name provided - runtime API key updates won't work
      - "my-api-key"
    retrievers:
      - kind: file
