...
The S3 exporter also works with S3-compatible object stores such as **MinIO**, Ceph or Garage: keep `kind: s3` and
point the AWS SDK at your server with `AWS_ENDPOINT_URL_S3`.

:::info
Read the [**MinIO** page](./minio.mdx) for the full setup, including the bucket addressing gotcha you will hit if your
endpoint is a hostname rather than an IP address.
:::
...
| `AwsConfig`               | <NotMandatory /> | `config.LoadDefaultConfig()`                                                                                                       | An instance of `aws.Config` that configures your access to AWS *(see [this documentation for more info](https://aws.github.io/aws-sdk-go-v2/docs/configuring-sdk/))*.<br/>If not set, `config.LoadDefaultConfig()` is called for you, which reads the standard AWS environment variables and shared config files.                                                                       |
