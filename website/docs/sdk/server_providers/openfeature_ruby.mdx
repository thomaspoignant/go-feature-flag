...
| `headers`  | A `Hash` object containing the headers to send to the relay-proxy.<br/>*(example to send APIKey: `{"X-API-Key" => "my-api-key"}` |
...
```ruby
require "openfeature/go_feature_flag"
