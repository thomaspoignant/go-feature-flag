...
If you are using the Azure blob storage, the easiest way to provide credentials is to set environment variables.
It will be used by GO Feature Flag to identify to your Azure blob storage container.
...
| `item`     | string | **none** | **(mandatory)** Path to the file inside the container _(ex: `config/flag/my-flags.yaml`)_.                              |
