	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				defaultValue: true,
				cacheMock: NewCacheMock(testflag.NewFlag(testflag.Data{
		...
		},
		{
			...
			args: args{
				...
				user:         ffuser.NewUser("random-key"),
				defaultValue: true,
				cacheMock:    NewCacheMock(testflag.NewFlag(testflag.Data{}), errors.New("flag [key-not-exist] does not exists")),
			},
			want:        true,
			wantErr:     true,
			expectedLog: "^\\[" + testutils.RFC3339Regex + "\\] user=\"random-key\", flag=\"key-not-exist\", value=\"true\"\n",
